...
if ( ! class_exists( 'TGM_Plugin_Activation' ) ) {
	...
	class TGM_Plugin_Activation {
		...
		/**
		 * Rename plugins directory to plugin slug
		 */
		public function rename_github_zip( $upgrade_dir, $remote_dir, $skin_upgrader ) {
			...
			if ( $skin_upgrader->bulk ) {
				...
				$slug = $this->prepare_slug( $skin_upgrader->skin->plugin_names[ $skin_upgrader->skin->i ] );
			...
			if (
				! isset( $skin_upgrader->skin->options['plugin'] )
				|| ! is_array( $skin_upgrader->skin->options['plugin'] )
			) {
				return $upgrade_dir;
			}

			$rewrite = ( isset( $skin_upgrader->skin->options['plugin']['slug'] ) )
						? $skin_upgrader->skin->options['plugin']['slug']
						: false;
		...
		/**
		 * Prepare plugin slug to rename by plugin name
		 */
		public function prepare_slug( $plugin_name ) {

			foreach ( $this->plugins as $plugin ) {

				if ( false === array_search( $plugin_name, $plugin ) ) {
					continue;
				}

				// Return plugin slug if this plugin found in registered
				if ( ! empty( $plugin['slug'] ) ) {
					return $plugin['slug'];
				}
			}

			// If plugin was not found - just replace spaces with '-' in name
			...
			class TGM_Bulk_Installer extends WP_Upgrader {
				...
				public function run( $options ) {
					...
					if ( ! $res ) {
						...
						//$working_dir = apply_filters( 'upgrader_source_selection', $working_dir, $destination, $this );
